...
from .utils import bytes_to_larger, humansize, rewrite_line
...
def labelIdsToLabels(labelIds):
    ...
    if allLabels == {}:
        results = callGAPI(
            gmail.users().labels(), "list", userId="me", fields="labels(id,name)"
        )
...
def restored_message(request_id, response, exception):
    ...
    try:
        sqlcur.execute(
            "INSERT INTO restored_messages (message_num) VALUES (?)", (request_id,)
        )
    except sqlite3.IntegrityError:
        pass
...
def backup_chat(request_id, response, exception):
    ...
    try:
        sqlcur.execute(
            """INSERT INTO messages (message_num, message_filename,
...
def getSizeOfMessages(messages, gmail):
    ...
    for message in messages:
        batch_request.add(
            gmail.users()
            .messages()
            .get(userId="me", id=message, format="minimal", fields="id,sizeEstimate")
        )
...
def restore_msg_to_group(gmig, full_message, message_num, sqlconn):
    ...
    try:
        ...
        result = callGAPI(
            gmig.archive(), "insert", groupId=options.email, media_body=media
        )
        if "responseCode" in result and result["responseCode"] == "SUCCESS":
            sqlcur = sqlconn.cursor()
            sqlcur.execute(
                "INSERT INTO restored_messages (message_num) VALUES (?)", (message_num,)
            )
            sqlconn.commit()
    except googleapiclient.errors.HttpError as e:
        print("ERROR: %s" % e)
