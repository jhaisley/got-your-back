...
from .utils import readFile, writeFile, systemErrorExit, _createHttpObj, getGYBVersion, shorten_url, _localhost_to_ip
...
''' % (auth_as, selected_scopes[0], selected_scopes[1], selected_scopes[2],
       selected_scopes[3], selected_scopes[4], selected_scopes[5], selected_scopes[6])
...
def writeCredentials(creds):
  ...
  auth_as = options.use_admin if options.use_admin else options.email
  cfgFile = os.path.join(options.config_folder, '%s.cfg' % auth_as)
  data = '{"token": "%s", "refresh_token": "%s", "id_token_jwt": "%s", "token_expiry": "%s", "client_id": "%s", "client_secret": "%s"}' % (
    creds.token, creds.refresh_token, getattr(creds, '_id_token', ''), 
    creds.expiry.strftime('%Y-%m-%dT%H:%M:%SZ') if creds.expiry else '1970-01-01T00:00:01Z',
...
class ShortURLFlow(google_auth_oauthlib.flow.InstalledAppFlow):
    def authorization_url(self, **kwargs):
        ...
        if shortened_url != auth_url:
          ...
          print('')
          print('If you use this machine\'s browser, click the Browser Auth button on the page.')
          print('If you use a different machine\'s browser, click the Console Auth button on the page.')
        else:
...
<body>
    ...
    <script>
        ...
        function submitCode() {
            ...
            if 'code' in parsed:
                ...
                response_headers = [('Content-type', 'text/html')]
                start_response(status, response_headers)
                return [b'<h2>Authorization successful!</h2><p>You can close this window.</p>']
            else:
    ...
    app = AuthApplication()
    httpd = wsgiref.simple_server.make_server(_localhost_to_ip(), 0, app, handler_class=AuthHandler)
...
def getSvcAcctCredentials(scopes, act_as):
  ...
  credentials = google.oauth2.service_account.Credentials.from_service_account_info(key_data, scopes=scopes)
  if act_as:
